...
function registerRoute(path, routeModule) {
    ...
    if (routeModule.GET)
        app.get(path, invoke(routeModule.GET));
    if (routeModule.POST)
        app.post(path, invoke(routeModule.POST));
    if (routeModule.PATCH)
        app.patch(path, invoke(routeModule.PATCH));
    if (routeModule.PUT)
        app.put(path, invoke(routeModule.PUT));
