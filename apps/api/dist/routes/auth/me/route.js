...
export async function GET(request) {
    ...
    const memberships = groupMemberships.map(({ group, membership }) => ({
        ...
        season: group.season,
        leagueName: leagueMap.get(group.competitionKey) ||
...
export async function PATCH(request) {
    ...
    try {
        ...
        const nextFavoriteTeam = typeof body.favoriteTeam === "string" || body.favoriteTeam === null ? body.favoriteTeam : undefined;
        ...
        if (nextUsername !== undefined) {
            ...
            if (!USERNAME_PATTERN.test(normalizedUsername)) {
                return jsonResponse({ error: "Username can only include letters, numbers, dot, underscore and dash" }, { status: 400 });
            }
        }
        if (nextEmail === null) {
