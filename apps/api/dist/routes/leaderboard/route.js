...
function cloneScoreMapByPeriod(source) {
    const next = new Map();
...
function longestConsecutive(values, predicate) {
    let current = 0;
    let best = 0;
...
function pickWinner(entries, score, tiebreak) {
    if (entries.length === 0) {
        return null;
    }
...
async function buildFixtureScoreMaps(input) {
    ...
    const scoreMapByPeriod = new Map();
...
function buildRows(params) {
    ...
    const rows = [...statsByUser.values()].map((entry) => {
...
function buildGroupStats(input) {
    ...
    let totalPoints = 0;
    const pointsByFechaUser = new Map();
    input.predictions.forEach((prediction) => {
        ...
        if (!pointsByFechaUser.has(prediction.period)) {
            pointsByFechaUser.set(prediction.period, new Map());
        }
    ...
    const worldBenchmark = leaderPoints && leaderPoints > 0
        ? {
...
function buildPeriodSnapshots(input) {
    ...
    orderedPeriods.forEach((period) => {
        ...
        const periodRows = buildRows({
            ...
            scoreMap: input.scoreMapByPeriod.get(period) || new Map()
        });
        snapshots.push({
            period,
            periodLabel: formatRoundLabel(period),
...
function buildStatsAwards(input) {
    const metricsByUser = new Map();
    const timelineByUser = new Map();
    const ensureTimeline = (key, name) => {
    ...
    input.periodSnapshots.forEach((snapshot) => {
        const periodRowsByUser = new Map(snapshot.rows.map((row) => [rowKey(row), row]));
        const allUserKeys = new Set([...timelineByUser.keys(), ...periodRowsByUser.keys()]);
    ...
    const bilardista = pickWinner(bilardistaPool.length > 0 ? bilardistaPool : entries, (entry) => entry.result, (a, b) => a.miss - b.miss || byName(a, b)) || fallbackEntry;
    ...
    const robinHood = pickWinner(entries, (entry) => entry.robinDifficultHits * 10 + entry.robinEasyFails, (a, b) => b.robinDifficultHits - a.robinDifficultHits || byName(a, b)) || fallbackEntry;
...
export async function GET(request) {
    ...
    const mode = modeParam === "stats" ? "stats" : "posiciones";
    const period = periodParam === "global" ? "global" : periodParam;
    ...
    const selectedMembership = (selectedGroupId ? memberships.find((item) => item.group.id === selectedGroupId) : null) || memberships[0];
    ...
    try {
        ...
        if (mode === "stats") {
            ...
            const allPeriodScoreMaps = allPeriods.length > 0
                ...
                : { scoreMap: new Map(), scoreMapByPeriod: new Map() };
        ...
        if (message.includes("PocketBase 403")) {
            return jsonResponse({
                error: "PocketBase rules are blocking group-wide leaderboard reads. Update predictions list/view rules to allow active group members to read group predictions."
            }, { status: 409 });
        }
