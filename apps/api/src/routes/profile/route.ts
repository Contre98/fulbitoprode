...
import type { ProfileActivityItem, ProfilePayload } from "@fulbito/server-core/types";
...
export async function GET(request: Request) {
  ...
  const fixtureMapsByCompetition = new Map(fixturesByCompetitionEntries.map(([key, value]) => [key, value] as const));
  ...
  const scoredActivitiesSorted = [...predictionActivities]
    .filter((item): item is ProfileActivityItem & { points: number } => typeof item.points === "number")
    .sort((a, b) => parseTimestamp(b.occurredAt) - parseTimestamp(a.occurredAt));
  let streak = 0;
  ...
  const achievements: NonNullable<ProfilePayload["achievements"]> = [];
  ...
  const targetCompetitionKey = targetMembership
    ...
    : "";
  const targetScoreMap = fixtureMapsByCompetition.get(targetCompetitionKey)?.scoreMap || new Map<string, { home: number; away: number }>();
  ...
  const periodPointsByUser = new Map<string, Map<string, number>>();
  targetGroupPredictions.forEach((prediction) => {
    ...
    const byPeriod = periodPointsByUser.get(prediction.userId) || new Map<string, number>();
  ...
  const targetPeriods = Array.from(
    new Set(
      targetGroupPredictions
        .map((prediction) => prediction.period)
        .filter((period): period is string => Boolean(period))
    )
  ).sort((a, b) => periodSortValue(a) - periodSortValue(b) || a.localeCompare(b));

  const cumulativeByUser = new Map<string, number>();
  const rankHistory: NonNullable<ProfilePayload["rankHistory"]> = [];
  ...
  let weeklyWinner: ProfilePayload["weeklyWinner"] = null;
  if (lastPeriod) {
